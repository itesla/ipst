...
start() {
    ...
    if [ -n "$pid" ]; then
        ...
        nohup $JAVA_HOME/bin/java -Xmx$java_xmx $options -cp $installDir/share/java -jar $installDir/share/java/histodb-server-0.2.0-exec.jar > $logsDir/histodb.log 2>&1&
        pid=$!
