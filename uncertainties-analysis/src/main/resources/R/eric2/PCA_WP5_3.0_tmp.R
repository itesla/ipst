...
################################################################
## calcul des composantes principales et des quantiles à 5 et  #
## 95 % de la diffence entre dacfs et snapshots à partir de la #
...
## difference entre dacf et snapshot à partir de la matrice    #
## d'entree                                                    #
################################################################
...
diff_dacfs_snapshots<-function(d_s,ncp) {
  ...
  date_horizon = cbind(mat[,"datetime"],mat[,"forecastTime"])


  mat <- mat[,colnames(mat)!="datetime"]

  mat<- mat[,colnames(mat)!="forecastTime"]

  mat<-cbind(date_horizon,mat)

  names(mat)[names(mat)=="2"]<-"forecastTime"

  names(mat)[names(mat)=="1"]<-"datetime"

  print(data.frame("debut extraction dacfs",date()))
  dacfs<-mat [mat [,"forecastTime"]%in%(1440),]

  date<-dacfs[,"datetime"]

  dacfs<-dacfs[,colnames(dacfs)!="datetime"]

  dacfs<-dacfs[,colnames(dacfs)!="forecastTime"]

  dacfs<-dacfs[,colnames(dacfs)!="date"]


  snapshots<-mat [mat [,"forecastTime"]%in%(0),]

  snapshots<-snapshots[,colnames(snapshots)!="datetime"]

  snapshots<-snapshots[,colnames(snapshots)!="forecastTime"]
...
#tri des variables dans dacfs ou snapshots
...
#### que les variables de départ sont les composantes principales#
##################################################################
