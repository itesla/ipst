...
###                   prct_inbox = the percentage of observations in the error box
...
##############################################################################################
###   Example:
###   box=error_box(errors_clust,prct_inbox,errors_cor,nb_PC)
##############################################################################################
...
error_box<-function(dataset,prct_risk,errors_cor,nb_PC){
  ...
  #Return results
  box_res=list(q_inf[1:nb_PC],q_sup[1:nb_PC],means,load[,1:nb_PC],data_PC[,1:nb_PC])
  names(box_res)=c("inf","sup","means","loadings","scores")
