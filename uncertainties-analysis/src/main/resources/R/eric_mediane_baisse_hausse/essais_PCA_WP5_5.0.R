...
################################################################
## definition du fichier csv a traiter                         #
################################################################
...
################################################################
## calcul des composantes principales et des quantiles a 5 et  #
## 95 % de la diffence entre dacfs et snapshots a partir de la #
...
## difference entre dacf et snapshot a partir de la matrice    #
## d'entree                                                    #
################################################################
...
#### que les variables de depart sont les composantes principales#
##################################################################
...
bornes_VDs<-function(M){
  ...
  for (i in 1:ncol(M)){
    ...
    distrib1pos<-distrib1[distrib1>0]

    distrib1neg<-distrib1[distrib1<0]


    fit1<-fitdistr(distrib1pos,"normal")
    ...
    inf<-qnorm(0.5, mean = c, sd = d)


    fit1<-fitdistr(distrib1neg,"normal")
