...
function exitcode=MCLA_HELPER(m1file,m3file, ofile, s_scenarios,option_sign,centerings,isdeterministics,full_deps)
close all;
mversion='1.8.1';
            ...
            for jcol = 1:size(inj_ID0,2)
                ...
                if isempty(idxgen)==0
                    if generatore(idxgen(1)).conn == 1
                        if strcmp(inj_ID0{jcol}(end),'P')
                            ...
                            if ismember(jcol,idx_fore0)
                                y0m(find(idx_fore0==jcol)) = generatore(idxgen(1)).P;
                                y1m(jcol)=generatore(idxgen(1)).P;
                            end
                        else
                            type_Xm(:,jcol) = [4;idxgen(1)];
                            if ismember(jcol,idx_fore0)
                                y0m(find(idx_fore0==jcol)) = generatore(idxgen(1)).Q;
                                y1m(jcol)=generatore(idxgen(1)).Q;
                            end
                        end
                ...
                if isempty(idxload)==0
                    if carico(idxload(1)).conn == 1 && carico(idxload(1)).P ~= 0 && carico(idxload(1)).Q ~= 0
                        ...
                        if ismember(jcol,idx_fore0)
                            y0m(find(idx_fore0==jcol)) = carico(idxload(1)).P;
                            y1m(jcol) = carico(idxload(1)).P;
                        end
                        if isempty(maxvalue)
                            limits_reactive(idxload(1),1:2)=[-9999 9999];
                        else
                ...
                if isempty(idxload)==0
                    if carico(idxload(1)).conn == 1
                        if strcmp(inj_ID0{jcol}(end),'P')
                            ...
                            if ismember(jcol,idx_fore0)
                                y0m(find(idx_fore0==jcol)) = carico(idxload(1)).Q;
                                y1m(jcol) = carico(idxload(1)).Q;
                            end
                        end
                    end
                end
...
% keyboard
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[PGENS PLOADS QLOADS ] = main_MCLA2PC3(generatore,carico,nodo,scenarios,type_X,type_Xm,module2,module3,flagPQ,limits_reactive,opt_sign,dati_condUNI,dati_condMULTI,y0,y0m,conditional_sampling,mod_gaussian,centering,mod_unif,full_dep);
