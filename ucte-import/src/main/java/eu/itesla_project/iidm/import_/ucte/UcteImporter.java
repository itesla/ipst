...
public class UcteImporter implements Importer {
    ...
    private static void createBuses(UcteNetworkExt ucteNetwork, Network network, EntsoeFileName entsoeFileName) {
    ...
    private static TwoWindingsTransformer createXnodeTransfo(UcteNetworkExt ucteNetwork, UcteTransformer ucteTransfo, boolean connected,
                                                             ...
                                                             Substation substation, EntsoeFileName entsoeFileName) {
        ...
        String yNodeName = entsoeFileName.getCountry() != null ? entsoeFileName.getCountry() + "_" + xNodeName : "YNODE_" + xNodeName;
    ...
    private static void createTransformers(UcteNetworkExt ucteNetwork, Network network, EntsoeFileName entsoeFileName) {
        for (UcteTransformer ucteTransfo : ucteNetwork.getTransformers()) {
            ...
            if (nodeCode1.getUcteCountryCode() == UcteCountryCode.XX &&
                ...
                transformer = createXnodeTransfo(ucteNetwork, ucteTransfo, connected, nodeCode1, nodeCode2, ucteVoltageLevel2, substation, entsoeFileName);

            } else if (nodeCode2.getUcteCountryCode() == UcteCountryCode.XX &&
                       nodeCode1.getUcteCountryCode() != UcteCountryCode.XX) {
                // transformer connected to XNODE
                transformer = createXnodeTransfo(ucteNetwork, ucteTransfo, connected, nodeCode2, nodeCode1, ucteVoltageLevel1, substation, entsoeFileName);
    ...
    public Network import_(ReadOnlyDataSource dataSource, Properties parameters) {
        try {
            ...
            try (BufferedReader reader = new BufferedReader(new InputStreamReader(dataSource.newInputStream(null, ext)))) {
                ...
                createBuses(ucteNetwork, network, entsoeFileName);
                createLines(ucteNetwork, network);
                createTransformers(ucteNetwork, network, entsoeFileName);
